...
export default function AdvicePreview(props) {
      ...
      <div className="Content-container">
        <Container className="Content-body" size="md" px="xs">
          <Card withBorder shadow="sm" radius="md">
            <div className="Articles-single" key={props.article.id}>
              Article #{props.article.id}{' '}
              <Title size={20}>{props.article.title}</Title>
              <Title size={12}>Written by: {props.article.author}</Title>
              <br />
            </div>
          </Card>
        </Container>
